#!/usr/bin/env python3
"""Experiment 55: VEP retention across codec compression tiers.

Measures how well OneEmbeddingCodec compression preserves variant-effect
prediction signal on ProteinGym DMS assays and ClinVar pathogenicity.
...
Usage:
    uv run python experiments/55_vep_retention.py --smoke-test --skip-clinvar --skip-rns
    uv run python experiments/55_vep_retention.py --skip-clinvar
    uv run python experiments/55_vep_retention.py  # full run
"""
...
ALL_CODEC_SPECS: dict[str, CodecSpec] = {
    ...
    "fp16_896":   CodecSpec(name="fp16_896",   d_out=896,  quantization=None),
    "int4_896":   CodecSpec(name="int4_896",   d_out=896,  quantization="int4"),
    "pq224_896":  CodecSpec(name="pq224_896",  d_out=896,  quantization="pq",  pq_m=224),
    "binary_896": CodecSpec(name="binary_896", d_out=896,  quantization="binary"),
}

DEFAULT_CODEC_NAMES = ["lossless", "fp16_896", "int4_896", "pq224_896", "binary_896"]
SMOKE_CODEC_NAMES = ["lossless", "binary_896"]
...
def main():
    parser = argparse.ArgumentParser(description="Exp 55: VEP retention across codec tiers")
    parser.add_argument(
        "--codecs",
        nargs="+",
    ...
    parser.add_argument(
        "--smoke-test",
        action="store_true",
        help="Reduced run: lossless + binary_896 only.",
    )
    ...
    if args.smoke_test:
        ...
        print("=== SMOKE TEST: lossless + binary_896 only ===")
    elif args.codecs:
    ...
    # ── Save JSON ──
    out_path = RESULTS_DIR / "exp55_vep_retention.json"
