...
def extract(input_path, output_path, batch_size=4, device=None, max_length=2000):
    ...
    Args:
        ...
        max_length: skip (NOT truncate) proteins longer than this
    """
    from src.extraction.prot_t5_extractor import extract_prot_t5_embeddings

    seqs = read_fasta(input_path)

    # Over-length proteins are skipped loudly inside the extractor (skip-not-
    # truncate); pass the cap through rather than silently pre-filtering here.
    embeddings = extract_prot_t5_embeddings(
        seqs, batch_size=batch_size, device=device, max_residues=max_length
    )
